...
def log_conversation(question: str, result: RagResult, response_time_ms: int) -> int:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            RETURNING id
            """,
            (question, result.answer.answer, result.model, result.prompt_tokens,
...
def save_feedback(conversation_id: int, value: int) -> None:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
